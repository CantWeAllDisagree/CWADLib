...
import cantwe.alldisagree.cwadlib.tests.events.OnApplySlow;
import cantwe.alldisagree.cwadlib.tests.events.OnFOVUpdate;
import cantwe.alldisagree.cwadlib.tests.events.OnHeldItemRender;
...
import cantwe.alldisagree.cwadlib.tests.packets.CPacketThrow.State;
			...
			if(!(serverplayer.getItemCooldownManager().getCooldownProgress(stack.getItem(), 1.0F) > 0 && cdConfig)) {
				...
				if(action.equals(CPacketThrow.State.FINISH) && data.getChargeTime() >= 0 ) {
					...
					if(baseThrow>0) {
						...
						ThrowMeEntity throwedEntity = new ThrowMeEntity(world, serverplayer, shouldDestroy, (float) totalDamage, stack.split(size));
