...
*/
package cantwe.alldisagree.cwadlib.throwme.entity.projectile;
...
public class ThrowMeEntity extends PersistentProjectileEntity implements FlyingItemEntity{


    private float clientSideRotation = 0;
    private boolean counterClockwiseBounce = true;

    private static final TrackedData<Byte> LOYALTY_LEVEL = DataTracker.registerData(ThrowMeEntity.class, TrackedDataHandlerRegistry.BYTE);
    private static final TrackedData<NbtCompound> COMPOUND_STACK = DataTracker.registerData(ThrowMeEntity.class, TrackedDataHandlerRegistry.NBT_COMPOUND);
    private static final TrackedData<BlockPos> DESTROYED_BLOCK = DataTracker.registerData(ThrowMeEntity.class, TrackedDataHandlerRegistry.BLOCK_POS);
    private static final TrackedData<Boolean> SHOULD_DESTROY = DataTracker.registerData(ThrowMeEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    public ThrowMeEntity(EntityType<? extends ThrowMeEntity> type, World worldIn) {
        super(type, worldIn);
    }

    public ThrowMeEntity(World worldIn, LivingEntity thrower, boolean canDestroy , float attackDamage, ItemStack thrownStackIn) {
        super(EntityReg.WEAPONTHROW, thrower, worldIn);
        this.attackDamage = attackDamage;
        this.dataTracker.set(COMPOUND_STACK, thrownStackIn.copy().writeNbt(new NbtCompound()));
        this.dataTracker.set(LOYALTY_LEVEL, (byte) ThrowMeEntity.getReturnOrLoyaltyEnchantment(thrownStackIn));
        this.dataTracker.set(DESTROYED_BLOCK, BlockPos.ORIGIN);
        this.dataTracker.set(SHOULD_DESTROY, canDestroy);
    }

    public ThrowMeEntity(World worldIn, double x, double y, double z) {
        super(EntityReg.WEAPONTHROW, x, y, z, worldIn);
    }
    ...
    public void tick() {
        ...
        int gravityWorld = EnchantmentHelper.getLevel(EnchantmentHandler.GRAVITY, this.getItemStack());
        ...
        int i = (int) this.dataTracker.get(LOYALTY_LEVEL);
    ...
    @Nullable
    protected EntityHitResult getEntityCollision(Vec3d Vec3pos1, Vec3d Vec3pos2) {
        return this.dealtDamage ? null : super.getEntityCollision(Vec3pos1, Vec3pos2);
    }
    ...
    protected void onEntityHit(EntityHitResult EntityHit) {
        ...
        if (entity instanceof LivingEntity livingEntity) {
            ...
            f += EnchantmentHelper.getLevel(EnchantmentHandler.THROW, this.getItemStack())*1F;
        ...
        if (entity.damage(damagesource, f)) {
            ...
            if (entity instanceof LivingEntity livingEntity1) {
                ...
                int contusionWorld = EnchantmentHelper.getLevel(EnchantmentHandler.CONCCUSION, this.getItemStack());
                if (contusionWorld > 0) {
                ...
                int groundedWorld = EnchantmentHelper.getLevel(EnchantmentHandler.GROUNDEDEDGE, this.getItemStack());
                ...
                if(this.getItemStack().getItem() instanceof BlockItem) {
                    ...
                    Item thrownItem = this.getItemStack().getItem();
                    if(thrownItem.equals(Items.BLAZE_ROD) || thrownItem.equals(Items.BLAZE_POWDER)) {
    ...
    public void readCustomDataFromNbt(NbtCompound compound) {
        ...
        this.dataTracker.set(LOYALTY_LEVEL, (byte) ThrowMeEntity.getReturnOrLoyaltyEnchantment(this.getItemStack()));
    ...
    protected void onCollision(HitResult result) {
        ...
        if (raytraceresult$type == HitResult.Type.ENTITY) {
            ...
            if(!state.getBlock().equals(Blocks.BEDROCK) && this.shouldDestroy()) {
                ...
                boolean canHarvest = this.getItemStack().isSuitableFor(state);
                if(canHarvest) {
